...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "address", nargs="+", type=str, help="The address to search for"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Library found with address "{address_to_search}": {library.name}'
                )
            )
        except Library.DoesNotExist:
            self.stdout.write(
                self.style.WARNING(
                    f'Library with address "{address_to_search}" not found in the database.'
                )
            )
