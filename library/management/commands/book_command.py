...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if hasattr(book, "publisher") and callable(
                getattr(book.publisher, "all", None)
            ):
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Publishers: {", ".join(publishers)}')
            )
        except Book.DoesNotExist:
            self.stdout.write(
                self.style.WARNING(
                    f'Book with title "{book_title}" not found in the database.'
                )
            )
