...
class AuthorSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if len(value) < 5:
            raise serializers.ValidationError("Длина поля 'name' должна быть не менее 5 символов.")
        return value
...
class LibrarySerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if len(value) < 10:
            raise serializers.ValidationError("Длина поля 'name' должна быть не менее 10 символов.")
        return value
...
class BookSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        if len(value) < 5:
            raise serializers.ValidationError("Длина поля 'title' должна быть не менее 5 символов.")
        return value
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    fields_errors={'blank': 'Это поле не может быть пустым.', 'invalid': 'Значение должно быть в диапазоне от 1 до 5.'}
    ...
    def validate_comment(self, value):
        if len(value) < 15:
            raise serializers.ValidationError("Длина поля 'comment' должна быть не менее 15 символов.")
        return value
    
    def validate_rating(self, rate):
        if not (1 <= rate <= 5):
            raise serializers.ValidationError("Значение поля 'rating' должно быть в диапазоне от 1 до 5.")
        return rate