...
class URIVariable:
    ...
    def _query_expansion(
        ...
        if list_test(value) and not tuples:
            ...
            if explode:
                return self.join_str.join(f"{name}={quote(v, safe)}" for v in value)
            else:
        ...
        if dict_test(value) or tuples:
            ...
            if explode:
                ...
                value = ",".join(f"{quote(k, safe)},{quote(v, safe)}" for k, v in items)
                return f"{name}={value}"
    ...
    def _label_path_expansion(
        ...
        if value is None or (
            not isinstance(value, (str, int, float, complex)) and len(value) == 0
        ):
            return None
