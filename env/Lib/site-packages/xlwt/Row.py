...
class Row(object):
    ...
    def get_row_biff_data(self):
        ...
        return BIFFRecords.RowRecord(
            self.__idx, self.__min_col_idx, self.__max_col_idx, height_options, options
        ).get()
    ...
    def set_cell_text(self, colx, value, style=Style.default_style):
        ...
        self.insert_cell(
            colx, StrCell(self.__idx, colx, xf_index, self.__parent_wb.add_str(value))
        )
    ...
    def set_cell_formula(self, colx, formula, style=Style.default_style, calc_flags=0):
        ...
        self.insert_cell(
            colx, FormulaCell(self.__idx, colx, xf_index, formula, calc_flags=0)
        )
        ...
        if isinstance(label, basestring):
            if len(label) > 0:
                self.insert_cell(
                    col,
                    StrCell(
                        self.__idx, col, style_index, self.__parent_wb.add_str(label)
                    ),
                )
            else:
    ...
    def __rich_text_helper(self, col, rich_text_list, style, style_index=None):
        ...
        for data in rich_text_list:
            if isinstance(data, basestring):
                ...
                if not isinstance(data[0], basestring) or not isinstance(data[1], Font):
                    raise Exception(
                        "Unexpected data type %r, %r" % (type(data[0]), type(data[1]))
                    )
                s = data[0]
        ...
        if rt:
            self.insert_cell(
                col, StrCell(self.__idx, col, style_index, self.__parent_wb.add_rt(rt))
            )
        else:
            self.insert_cell(col, BlankCell(self.__idx, col, style_index))
