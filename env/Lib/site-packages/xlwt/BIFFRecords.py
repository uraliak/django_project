...
class Biff8BOFRecord(BiffRecord):
    ...
    def __init__(self, rec_type):
        ...
        self._rec_data = pack(
            "<4H2I", version, rec_type, build, year, file_hist_flags, ver_can_read
        )
...
class BoundSheetRecord(BiffRecord):
    ...
    def __init__(self, stream_pos, visibility, sheetname, encoding="ascii"):
        ...
        self._rec_data = pack(
            "<LBB%ds" % uusheetname_len, stream_pos, visibility, 0x00, usheetname
        )
...
class RowRecord(BiffRecord):
    ...
    def __init__(self, index, first_col, last_col, height_options, options):
        self._rec_data = pack(
            "<6HL", index, first_col, last_col + 1, height_options, 0x00, 0x00, options
        )
    ...
    def __init__(self, merged_list):
        ...
        while i >= 0:
            ...
            while (i >= 0) and (j < 0x403):
                ...
                i -= 1
                j += 1
            self._rec_data += pack("<3H", self._REC_ID, len(merged) + 2, j) + merged
...
class FormulaRecord(BiffRecord):
    ...
    def __init__(self, row, col, xf_index, rpn, calc_flags=0):
        self._rec_data = (
            pack("<3HQHL", row, col, xf_index, 0xFFFF000000000003, calc_flags & 3, 0)
            + rpn
        )
...
class GutsRecord(BiffRecord):
    ...
    def __init__(
        self, row_gut_width, col_gut_height, row_visible_levels, col_visible_levels
    ):
        self._rec_data = pack(
            "<4H", row_gut_width, col_gut_height, row_visible_levels, col_visible_levels
        )
