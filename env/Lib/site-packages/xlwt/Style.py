...
def add_palette_colour(colour_str, colour_index):
    if not (8 <= colour_index <= 63):
        raise Exception(
            "add_palette_colour: colour_index (%d) not in range(8, 64)" % (colour_index)
        )
...
def _parse_strg_to_obj(
    ...
    for line in _esplit(strg, line_sep, esc_char):
        ...
        for counter in range(2):
            ...
            raise EasyXFAuthorError(
                "Attempt to define synonym of synonym (%r: %r)" % (old_section, result)
            )
        section_dict = result
        ...
        for kv_str in _esplit(item_str, field_sep, esc_char):
            ...
            for counter in xrange(2):
                ...
                raise EasyXFAuthorError(
                    "Attempt to define synonym of synonym (%r: %r)" % (old_k, result)
                )
            value_info = result
            ...
            for value_rule in value_info:
                if isinstance(value_rule, dict):
                    ...
                    raise EasyXFAuthorError(
                        "unknown value rule for attribute %r: %r" % (k, value_rule)
                    )
            else:
                raise EasyXFCallerError(
            ...
            try:
                ...
                raise EasyXFAuthorError(
                    "%s.%s in dictionary but not in supplied object" % (section, k)
                )
            if debug:
