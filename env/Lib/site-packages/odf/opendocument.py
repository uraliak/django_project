...
class OpenDocument:
    ...
    def __zipwrite(self, outputfp):
        ...
        if self.thumbnail is not None:
            ...
            self.manifest.addElement(
                manifest.FileEntry(fullpath="Thumbnails/thumbnail.png", mediatype="")
            )
        ...
        for op in self._extra:
            ...
            self.manifest.addElement(
                manifest.FileEntry(fullpath=op.filename, mediatype=op.mediatype)
            )
            if sys.version_info[0] == 3:
    ...
    def _saveXmlObjects(self, anObject, folder):
        ...
        if self == anObject:
            ...
            self.manifest.addElement(
                manifest.FileEntry(fullpath=folder, mediatype=anObject.mimetype)
            )
        # Write styles
        self.manifest.addElement(
            manifest.FileEntry(fullpath="%sstyles.xml" % folder, mediatype="text/xml")
        )
        ...
        # Write content
        self.manifest.addElement(
            manifest.FileEntry(fullpath="%scontent.xml" % folder, mediatype="text/xml")
        )
    ...
    for mentry, mvalue in manifest.items():
        ...
        # Load subobjects into structure
            ...
            if mvalue["full-path"][-1] == "/":
                doc._extra.append(
                    OpaqueObject(mvalue["full-path"], mvalue["media-type"], None)
                )
            else:
                doc._extra.append(
                    OpaqueObject(
