...
SUPBOOK_UNK, SUPBOOK_INTERNAL, SUPBOOK_EXTERNAL, SUPBOOK_ADDIN, SUPBOOK_DDEOLE = range(
    5
)
...
def open_workbook_xls(
    ...
    try:
        ...
        if biff_version not in SUPPORTED_VERSIONS:
            raise XLRDError(
                "BIFF version %s is not supported" % biff_text_from_num[biff_version]
            )
...
class Book(BaseObject):
    ...
    def get_sheets(self):
        ...
        if DEBUG:
            print(
                "GET_SHEETS:", self._sheet_names, self._sh_abs_posn, file=self.logfile
            )
    ...
    def handle_externsheet(self, data):
        ...
        if self.biff_version >= 80:
            ...
            if blah2:
                ...
                print(
                    "   %3d chars, type is %d (%s)" % (nc, ty, msg), file=self.logfile
                )
            if ty == 3:
    ...
    def handle_sheethdr(self, data):
        ...
        if DEBUG:
            print(
                "SHEETHDR: posn after get_sheet() =", self._position, file=self.logfile
            )
    ...
    def handle_sst(self, data):
        ...
        while 1:
            ...
            if DEBUG >= 2:
                fprintf(
                    self.logfile, "CONTINUE: adding %d bytes to SST -> %d\n", nb, nbt
                )
            strlist.append(data)
    ...
    def getbof(self, rqd_stream):
        ...
        if not (4 <= length <= 20):
            bof_error(
                "Invalid length (%d) for BOF record type 0x%04x" % (length, opcode)
            )
